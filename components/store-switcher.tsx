                ...
                <CommandList>
                    ...
                    <CommandGroup heading="Available stores">
                        {formattedItems.map(item=> (
                            <CommandItem key={item.value} onSelect={()=> onStoreSelect(item)} className="text-sm cursor-pointer">
                                <StoreIcon className="mr-3 w-4 h-4" />
                                <span>{item.label}</span>
                                <Check className={cn("ml-auto h-4 w-4", item?.value ===currentStore?.value ? "opacity-100" : "opacity-0" )} />
                            </CommandItem>
                        ))}
                    </CommandGroup>
                </CommandList>
